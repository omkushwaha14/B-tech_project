...
const Checkout = ({ products, setRun = (f) => f, run = undefined }) => {
  ...
  const showCheckout = () => {
    return getTotal() >= 49 ? <div>{delivarydetails()}</div> : <div></div>;
  };
  ...
  const buy = (e) => {
    ...
    const createOrderData = {
      ...
      phone: phonenumber,
      exactaddress: mainaddress,
    };

    createOrder(userIds, createOrderData).then((response) => {
      emptyCart(() => {
        setRun(!run);
        ...
        <form onSubmit={buy}>
          ...
          <div className="form-group">
            ...
            <small className="text-muted">
              Eg.Rohini sec-16 , dtu
            </small>
          </div>

          <button
            style={{ backgroundColor: "#008ECC", color: "white" }}
  ...
  return (
    <div>
      ...
      {!data.success && <h5>Total Amount: Rs {getTotal()}</h5>}
